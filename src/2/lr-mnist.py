...
class lr_mnist:
    ...
    def lr_classifier(self):
        ...
        '''
        self.clf = LogisticRegression(self.args.penalty='none', # No penalty applied. Here you could have added either L1, L2 or elastic net as a penalization method
    ...
    def predict_unseen(self):
        ...
        if self.args.predict_mode == 'Yes':
            classes = sorted(set(self.y)) # Defining unique set of labels in the digit data set
            new_image_path = self.args.new_image
            test_image = cv2.imread(new_image_path) # Loading the test image
            gray = cv2.bitwise_not(cv2.cvtColor(test_image, cv2.COLOR_BGR2GRAY)) # Applying COLOR_BGR2GRAY to make the test image greyscale
...
def main(): 
   ...
   # I try to make it possible executing arguments from the terminal
    ...
    ap.add_argument('-pen', # Argument 3
                    ...
                    default = 'none',
                    help='Penalty in Logistic Regression - default is 0.1')    
